...
description: >
  ...
  the current conversation. If the agentibridge MCP is unavailable, falls back to
...
This skill is paired with the **`agentibridge`** MCP server (registered in
`~/.claude.json` by `agentibridge install`). The primary path calls its
`handoff` tool — exposed to Claude Code as **`mcp__agentibridge__handoff`**.

Before attempting the call, confirm the tool is loaded in the current session.
If `mcp__agentibridge__*` tools are not visible, skip directly to
Step 4-fallback — do NOT try alternative server names (e.g.
...
Call **`mcp__agentibridge__handoff`** with:
...
This is the existing memory-file pipeline. Used when `agentibridge` is not
...
**Path:** Memory file (agentibridge unavailable)
...
- **`mcp__agentibridge__handoff` not loaded in session** → silent fallback to memory-file path. Suggest `agentibridge install` to the operator only if they ask why the MCP path was skipped.
